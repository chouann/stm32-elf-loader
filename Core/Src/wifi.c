...
static SemaphoreHandle_t wifi_uart_mutex = NULL;
static StreamBufferHandle_t wifi_rx_stream = NULL;
uint8_t dma_tmp_buffer[RX_BUFFER_SIZE];           // Receive buffer
...
/* Internal Reset Function (for use within wifi.c) */
...
int wifi_receive_file()
{
    ...
    while (HAL_GetTick() < timeout) {
        ...
        if (received > 0) {
            ...
            myprintf("%s", (char *)(temp_buffer + temp_len - received));
            // the format of message containing filename would be "FNAME=AAA.o,FSIZE=450"
            if (!f_is_open) {
                if (rx_flags.has_fname && rx_flags.has_fsize) {
                    ...
                    if(filesize == 0)
                        return -1;
                    myprintf("[wifi_receive_file] Try to open %s, size %d\r\n", filename, filesize);
                    
                    res = f_open(&file, filename, FA_CREATE_ALWAYS | FA_WRITE);
                    if(res == FR_OK) {
                        f_is_open = 1;
                        myprintf("[wifi_receive_file] f_open Succeeded\r\n");
                    } else {
                        myprintf("[wifi_receive_file] ERROR: f_open failed. res=%d\r\n", res);
                        return -1;
                    }
                    temp_len = 0; 
                    continue; 
            ...
            if (f_is_open && filesize > 0) {
                myprintf("[wifi_receive_file] f Wating for ipd ...\r\n");
                while (rx_flags.has_ipd) {
                    myprintf("[wifi_receive_file] Got ipd ...\r\n");
                    ...
                    if (current_msg_len >= expected_data_len) {
                        ...
                        if (res != FR_OK || expected_data_len != byteswrite) {
                            myprintf("[wifi_receive_file] ERROR: f_write failed or incomplete. res=%d, wrote=%u, expected=%u\r\n", res, byteswrite, expected_data_len);
                            f_close(&file);
                            return -1;
                        }
                        
                        filesize -= byteswrite;
                        myprintf("[wifi_receive_file] f_write succeeded, res=%d, wrote=%u, expected=%u, remaining_filesize=%u\r\n", res, byteswrite, expected_data_len, filesize);
                        ...
                        // Re-evaluate flags for the shifted buffer
                        wifi_detect_response_markers(temp_buffer, temp_len, &rx_flags);
                        myprintf("[wifi_receive_file] Keep waiting next ipd ...\r\n");
...
int wifi_receive_msg()
{
    ...
    while (HAL_GetTick() < timeout) {
        ...
        if (received > 0) {
            ...
            if (rx_flags.has_closed && rx_flags.has_ipd) {
                return 0;
            }
