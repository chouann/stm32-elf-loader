...
int main(void)
{
    ...
    // 3. Wait for ESP to complete boot messages (typically takes 2-3 seconds)
    HAL_Delay(3000);
    /* ESP8266 Init END */
    /* USER CODE END 2 */
    ...
    if (kernel_init() != 0 ||
        xTaskCreate(shell_task, "shell", 1024, NULL, 2, NULL) != pdPASS)
        Error_Handler();
...
static void MX_GPIO_Init(void)
{
    ...
    /*Configure GPIO pin Output Level */
    HAL_GPIO_WritePin(GPIOB, SD_CS_Pin | ESP8266_CH_PD_Pin | ESP8266_RST_Pin,
                      GPIO_PIN_RESET);
...
void Error_Handler(void)
{
    ...
    /* Blink the red LED so a boot failure is visible on the board.
     * Interrupts are off, so busy-wait instead of HAL_Delay(). */
    __disable_irq();
    while (1) {
